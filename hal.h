...
class Display {
 public:
  ...
  MOCK_METHOD(void, drawFastVLine, (int16_t x, int16_t y, int16_t h, uint16_t color));
  MOCK_METHOD(void, drawFastHLine, (int16_t x, int16_t y, int16_t w, uint16_t color));
...
HAL(Display *d, std::initializer_list<int> button_pins) : d(d) {
  ...
  std::for_each(button_pins.begin(), button_pins.end(), [](int pin){pinMode(pin, INPUT_PULLUP);});
}
